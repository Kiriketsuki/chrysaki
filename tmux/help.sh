...
# Ensure 24-bit color sequences render correctly in the popup environment
...
render_help() {
  ...
  printf "  ${PR}v${RS}                  ${SC}Start character selection${RS}\n"
  printf "  ${PR}Ctrl + v${RS}           ${SC}Toggle rectangle (block) selection${RS}\n"
  printf "  ${PR}y${RS}                  ${SC}Copy selection to system clipboard${RS}\n"
  printf "  ${PR}Enter${RS}              ${SC}Copy selection to system clipboard${RS}\n"
  printf "  ${PR}/ (in copy mode)${RS}   ${SC}Search backward${RS}\n"
  printf "  ${PR}Esc / q${RS}            ${SC}Exit copy mode${RS}\n"
  printf "  ${SC}Mouse drag selects and copies to clipboard on release.${RS}\n"
  printf "\n"
